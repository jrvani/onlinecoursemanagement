...
class AssignmentTest {
	...
	@Captor
	ArgumentCaptor<Assignment> argumentCaptorForAss;
	...
	void deleteTest()
	{
		...
		doNothing().when(assignmentDAO).delete(assignment1);
		AssignmentService ass=new AssignmentService(assignmentDAO);
		ass.deleteAssignments(course1, "java 1");
		verify(assignmentDAO,times(1)).delete(argumentCaptorForAss.capture());
		assertEquals("java 1",argumentCaptorForAss.getValue().getAssignmentName());
	}
